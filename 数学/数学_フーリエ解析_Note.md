# フーリエ解析
フーリエ解析に関しての基本事項を記載したマイノートです。<br>

![image](https://user-images.githubusercontent.com/25688193/46400070-7205a700-c734-11e8-87f7-9868ef321288.png)<br>
...
## ■ 概要
フーリエ解析は、フーリエ級数やフーリエ変換をツールとして、偏微分方程式の解析解を解いたり、時系列データのスペクトル解析に応用したりする解析手法の総称である。<br>

ここでは、基本的に、以下のトピックでフーリエ解析がどのようなものなのかを見ていくことにする。<br>

- 任意の（区分的に滑らかな）周期関数は、無限個の sin, cos の和で表現できることを主張したフーリエ級数展開<br>
- フーリエ級数展開の収束性について議論で、どのような場合にフーリエ級数が収束するのか？<br>
- より計算の簡単な複素フーリエ級数<br>
- 関数解析視点でのフーリエ級数として、フーリエ級数が完全直交系をなすこと。<br>
- フーリエ級数の周期を無限大に伸ばし、非周期関数に適用出来るようにしたフーリエ変換。<br>
- フーリエ級数、フーリエ変換の応用としての、偏微分方程式の解析解の解法<br>
- フーリエ級数、フーリエ変換の応用としての、スペクトル解析<br>
- 離散化されたデータ（＝時系列データ等）に対する、フーリエ級数展開とフーリエ変換<br>


<a id="ID_2"></a>

## ■ フーリエ級数展開 [Fourier series]
フーリエ級数、フーリエ級数展開は、（区分的に滑らかな）”任意の” 周期的な関数は、無限個の sin,cos の和で表現できることを主張したものである。<br>
以下、そのことを見ていく。<br>

まず、以下のような、sin,cos の和からなる級数（フーリエ級数）![image](https://user-images.githubusercontent.com/25688193/42722784-4ed96e1a-878d-11e8-889e-2832b4435f68.png) の式<br>
