...
- その後のレイヤーは、検出のための補助的な構造であり、主な特徴 [key features] は以下のようになる。<br>
    - 直感的には、<br>
        ...
        ![image](https://user-images.githubusercontent.com/25688193/39472033-9f557546-4d81-11e8-894a-bc1467b41665.png)<br>
    ...
    - **「検出のための、畳み込み予想器」**<br>
		...
		このカーネルは、予想カテゴリのスコア（正解率）、又は、デフォルトのバウンディングボックスの座標に関してのオフセット値を生成（算出）する。<br>
		又、バウンディングボックスの座標オフセットの出力値は、各特徴マップの位置に対するデフォルトのバウンディングボックスの位置に対して測定される。<br>
	
		SSD モデルは、（前述のように）いくつかの特徴レイヤーを、ベースネットワークの最後に追加するが、<br>
		**これらの特徴レイヤーは、異なるスケールとアスペクト比のデフォルトバウンディングボックスに対するオフセット値と、それに付随する確信度を予測する。**<br>

    - **「デフォルトバウンディングボックスとアスペクト比」**<br>
    ネットワークのトップで、複数の特徴マップに関して、各特徴マップのセル（グリッド）とデフォルトバウンディングボックスの集合を関連付けている。<br>
...
- **「デフォルトボックスについて」**
    ...
    いくつかの特徴マップでの各位置（左上座標）において、異なるアスペクト比デフォルトボックスの少数の集合（上記例では４個）を、<br>
