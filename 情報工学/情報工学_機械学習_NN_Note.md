...
- その後のレイヤーは、検出のための補助的な構造であり、主な特徴 [key features] は以下のようになる。<br>
    - 直感的には、<br>
        ...
        ![image](https://user-images.githubusercontent.com/25688193/39513192-7d72b67a-4e2e-11e8-9125-172ece027ab9.png)<br>
    ...
    - **「検出のための、畳み込み予想器」**<br>
		...
		このカーネルは、予想カテゴリのスコア（正解率）、又は、デフォルトのバウンディングボックス（オフセット前のバウンディングボックス）の座標に関してのオフセット値を生成（算出）する。<br>
		又、バウンディングボックスの座標オフセットの出力値は、各特徴マップの位置に対するデフォルトのバウンディングボックスの位置（オフセット前のバウンディングボックス）に対して測定される。<br>
    ...
    - **「デフォルトボックス（オフセット前のバウンディングボックス）とアスペクト比」**<br>
    	...
    	各特徴マップのセル（グリッド）において、物体の所属クラスが、それら（物体）の各ボックス内に存在することを示す指標である<br>
	    所属クラス毎のスコア（正解率）と同様に、セル中のデフォルトボックスの座標オフセット値を予想する。<br>
	    具体的には、ある座標位置で k 個のボックスそれぞれに対し、<br>
	    c 個のクラススコア（所属クラスの確率）と、元のデフォルトボックスに対するオフセット値４つを計算する。<br>
	    その結果、特徴マップに適用されるフィルタは、合計 (c+4)×k 個となり、<br>
	    m×n の特徴マップに対して、(c+4)×k×mn 個の出力が生成されることになる。<br>
		
    	幾つかの特徴マップで、異なるデフォルトボックスの形状を利用することにより、<br>
	    可能な出力されるボックスの形状を、効率よく離散化 [discretize] することが出来る。<br>

- **「デフォルトボックス（オフセット前のバウンディングボックス）について」**
    ![image](https://user-images.githubusercontent.com/25688193/39513225-9b26e0ce-4e2e-11e8-81fe-544dad110581.png)<br>
...
- **「マッチング戦略」**<br>
	訓練では、どのデフォルトボックスが正解の検出と対応するのか決定する必要があり、ネットワークをそれに応じて学習させる。<br>
	各正解ボックスは、座標位置、アスペクト比、スケール値が異なるデフォルトボックスから選択する。<br>
	
    デフォルトボックスに対して、jaccard overlap （下図）の最良値で、各正解ボックスのマッチ度を算出することになる。<br>
    ![image](https://user-images.githubusercontent.com/25688193/39512883-7067f2f2-4e2d-11e8-8497-ac39f2aaba7c.png)<br>
	この際、デフォルトボックスを jaccard overlap が 0.5 の値よりも大きい正解ボックス全てとマッチさせる。
...
- **「デフォルトバウンディングボックスとアスペクト比の選択」**<br>
    ...
    ![image](https://user-images.githubusercontent.com/25688193/39513260-c144f76e-4e2e-11e8-9adf-85c01dbd074c.png)<br>

	又、**SSD では、特定の特徴マップが、特定のスケールの物体に対応するように学習させるために、**
	**デフォルトバウンディングボックスを ”敷き詰めて” 設計されている。**<br>
		
	具体的には、今、m 個の特徴マップを予想に使用するケースにおいて、<br>
	各特徴マップ k についてのデフォルトバウンディングボックスのスケール ![image](https://user-images.githubusercontent.com/25688193/39475869-f53f3b46-4d94-11e8-8c20-3ca89df948d3.png) は、以下のようにして計算される。<br>
	...
	又、デフォルトバウンディングボックスのアスペクト比に関しては、![image](https://user-images.githubusercontent.com/25688193/39479352-f8164624-4d9f-11e8-9ed2-06a2557941d2.png) の異なるアスペクト比を設定する。<br>
	このスケールとアスペクト比により、各デフォルトバウンディングボックスに対して、<br>
