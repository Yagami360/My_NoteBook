...
- マージン最大化を行う線形識別器：<br>
    ...
    サポートベクターマシンは、線形識別器の１種であるが、マージンと呼ばれる線形識別器（＝分離超平面になる）からのノイズを取り除くための余白領域を最大化するように、線形識別器（＝ハードマージンSVM）を構成する。<br>
...
- カーネル法とカーネルトリック：<br>
    ...
    サポートベクターマシンでも他のカーネル法と同様にして、ユークリッド空間において線形分離不可能な問題を、より高次元の再生核ヒルベルト空間内での線形識別問題に置き換える。<br>
    そして、カーネルトリックにより、カーネル関数の計算を有限次元の計算で計算可能とする。<br>
...
> サポートベクターマシンの定式化は、マージン最大化を実現する線形識別器→スラッグ変数の導入→最適化問題への置き換え→、、、という順に議論していくスタイルが基本であるが、直接ヒンジ損失関数の正則化の議論から定式化する方法もある。<br>
...
が成り立つ。<br>
両辺を h で割って、線形識別関数の係数ベクトルとバイアス項を正規化し、それらを同じ記号で表記すると、![image](https://user-images.githubusercontent.com/25688193/49069241-1eaf6100-f26c-11e8-9e5c-e501061e88b3.png)<br>
