...
2. 【カーネルトリック】<br>
    ...
    尚、このカーネルトリックは、Moore - Aronszajn の定理からの帰着で自然に導かれる。<br>
...
4. 【カーネル法を利用した各種データ解析手法に共通する手順】<br>
    ...
    ３つ目のポイントは、このリプレゼンター定理により有限次元化されたデータ解析手法において、分散共分散行列などの計算で、内積演算が必要になるケースが多々あるが、この再生核ヒルベルト空間上での内積演算を２つの特徴ベクトルの内積を計算することなく、カーネル関数で表せるという、カーネルトリックを用いて、計算負荷を削減する点にある。<br>
