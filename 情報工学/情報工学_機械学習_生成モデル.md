...
- フローベースの生成モデルの基本的な考え方とアーキテクチャ：<br>
    ...
    従って、フローベースの生成モデルにおける（元の観測データ x への）推論は、VAE のように decoder を用いなくとも、形式的には以下の図のように、可逆な逆写像 ![image](https://user-images.githubusercontent.com/54523274/64470846-922fb880-d184-11e9-9eae-1748e4cbedda.png) によって直接的に行えるようになる。<br>

    > フローベースの生成モデルのアーキテクチャ図を要追加 ...

    ![image](https://user-images.githubusercontent.com/54523274/64470859-b12e4a80-d184-11e9-9a41-db5d6c5fc921.png)<br>

    ※ 但し、後述で述べるように、この写像 f を直接計算することは実際上には困難であるので、部分的な写像 f_1,f_2 ,..., f_K を繰り返して合成写像とすることで、徐々に目的の写像 f に近づけて行くことになる。<br>
    このような部分的な写像の繰り返し（＝合成写像）をフロー [flow] と呼ぶ。
