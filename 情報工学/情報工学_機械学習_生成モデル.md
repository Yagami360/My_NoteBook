...
➁ 識別器が、偽物画像 ![image](https://user-images.githubusercontent.com/25688193/57116920-36e35900-6d93-11e9-89d9-040c8a126c56.png) を（誤って）偽物だと判断し、D(G(z;θ_g );θ_d )=1.0 を出力する。
...
- ピクセル単位の Noise Input と Stochastic variation（確率的変動）：<br>
    ...
    - ノイズの影響と効果：<br>
        ...
        (a) 全解像度スケールの層に、ノイズを加えた場合<br>
        (b) ノイズを加えない場合<br>
        (c) 高解像度の層（64×64 ~ 1024×1024）のみに、ノイズを加えた場合(d) 低解像度の層（4×4 ~ 32×32）のみに、ノイズを加えた場合<br>
