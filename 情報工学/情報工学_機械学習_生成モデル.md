...
- ベイズ推定：<br>
    ...
    ![image](https://user-images.githubusercontent.com/25688193/62002522-c7d29200-b140-11e9-8db7-8d9d7ee1b05b.png)<br>
    ここで、この確率モデル p(x,z) における潜在変数 z の事後分布 p(z|x) を、以下で定義されるベイズの定理を用いて推定することを考える。<br>
    ![image](https://user-images.githubusercontent.com/25688193/62002523-cf923680-b140-11e9-9274-bf71aacf879b.png)<br>
	このようなベイズの定理を用いて、確率モデル（＝ベイズモデリング）の潜在変数 z の事後分布 p(z|x) を求める手法やこの事後分布を最大化するパラメータを求める手法をベイズ推定という。<br>

	ここで、観測データ x や潜在変数 z が共に、ガウス分布に従うような場合は、このベイズ定理の分母の積分 ∫ p(x|z)dz を解析的に求めることが可能となり、事後分布 p(z|x)  の解析解を得ることが出来る。（※ このような解析解で求まる場合 をclosed form で解けるともいう。）<br>

    しかしながら、一般的には、このように事後分布の解析解が得られるケースは稀で、そのため、数値計算的に近似解を求めることになる。<br>

- 最尤推定：<br>
	ベイズの定理において、尤度 p(x|z) を最大化するパラメータを求める手法。<br>
	※ これに対しベイズ推定は、尤度 p(x|z) ではなく、事後分布 p(z|x) を最大化するパラメータを求める手法。<br>
...
- MAP 推定（最大事後確率推定）：<br>
	観測データ x のサイズが十分に大きく、事後確率 p(z|x)  がデルタ関数のように局在化している場合において、事後確率を、<br>
