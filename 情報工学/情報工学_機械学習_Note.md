...
2. [モデルの評価 [evaluate model]](#ID_3)
    ...
    1. [適合率、再現率、F1 スコア](#ID_3-4)
    1. [混同行列（真陰性 : TN、真陽性 : TP、偽陽性 : FP、偽陰性 : FN）](#ID_3-5)
    1. [ROC 曲線 [receiver operator characteristics curve]](#ID_3-6)
...
1. [確率モデルと識別関数](#ID_4)
    1. [確率モデル](#ID_4-1)
    1. [ベイスの識別規則 [Bayes's idification rule]](#ID_4-2)
    1. [線形識別関数 [liner discriminant function]](#ID_4-3)
1. [線形判別分析 [LDA : liner discrinant analysis]](#ID_x)
1. [ロジスティクス回帰 [Logistic Regression]](#ID_x)
1. [最近接法, k-NN 法 [k-nearest neighbor algorithm]](#ID_x)
1. [サポートベクターマシン [SVM : Support Vector Machine]](#ID_x)
1. [決定木 [Decision Tree]](#ID_x)
1. [アンサンブル学習](#ID_x)
    1. [混合モデル](#ID_x-x)
    1. [EM アルゴリズム](#ID_x-x)
    1. [アダブースト [AdaBoost]](#ID_x-x)
    1. [バギング [Bagging]](#ID_x-x)
1. [クラスタリング [clustering]](#ID_x)
