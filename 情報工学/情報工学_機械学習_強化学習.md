...
![image](https://user-images.githubusercontent.com/25688193/49741784-84730280-fcda-11e8-8c5e-178aec9c9ca4.png)<br>
...
ここで、この関係式は、状態価値関数に関しての方程式になっており、ベルマンの方程式という。<br>
そして、このベルマンの方程式は不動点方程式であるので、この方程式を解けば、原理的には状態価値関数の最適解が存在することになる（詳細は後述）<br>
...
![image](https://user-images.githubusercontent.com/25688193/49683711-8d15de00-fb0c-11e8-94cc-90e8712b9f72.png)<br>

![image](https://user-images.githubusercontent.com/25688193/49740807-5ee4f980-fcd8-11e8-85db-9af73cf9e5ee.png)<br>
...
この図より、状態行動関数の定義にある割引総和（＝収益、利得）の期待値 ![image](https://user-images.githubusercontent.com/25688193/49740874-8b007a80-fcd8-11e8-9264-6881d4942257.png) は、オレンジ色内での報酬 r の平均をとったものになっていることが分かる。<br>
...
![image](https://user-images.githubusercontent.com/25688193/49683811-1679e000-fb0e-11e8-881f-9770674462dd.png)<br>
...
### ◎ ベルマン方程式
先に見たように、行動価値関数 Q^*(s,a) に関してグリーディな方策（＝状態 s において Q(s,a) を最大化する行動選択）は最適であるので、最適方策 π^* を求めるには、最適行動価値関数 Q^*(s,a) の情報、或いは、最適状態価値関数 V^*(s)・即時報酬 r・遷移確率 P の情報があればよいことになる。<br>

従って、次の問題は、これら最適価値関数 Q^*(s,a),V^*(s) を如何にして求めるかということになる。<br>
最適価値関数 Q^*(s,a),V^*(s) は、以下のベッセルの方程式と呼ばれる、最適解に関しての不動点方程式から求めることが原理的には可能となる。<br>
...
まずは、最適価値関数のうち、最適状態価値関数 V^*(s) に対してのベッセルの方程式を考える。

【定理】（最適状態価値関数に関しての）ベッセルの方程式<br>
マルコフ決定過程 M=(S,A,P) において、<br>
初期状態 s_0 と定常方策 \pi\in \Pi_stat から生じる確率過程 {s_t,a_t,r_t}_t=0^\infty に対して、状態価値関数 V^\pi(s) が定まるが、<br>
この最適行動価値関数 V^*(s) に対しては、以下のベルマンの方程式と呼ばれる関係が成り立つ。<br>
V^*(s) = max_(a\inA){ R(s,a) + \gamma * Σ_(s'\inS)( P(s,a,s')*V^*(s') ) } (\forall s\in S)<br>

ここで、このベルマンの方程式において状態価値関数を与える作用素を<br>
(T^\pi V) : s\in \doubleR^S → max_(a\inA){ R(s,a) + \gamma * Σ_(s'\inS)( P(s,a,s')*V^*(s') ) } \in \doubleR^S<br>
を、ベルマン最適作用素という。このベルマン最適作用素を用いれば、ベッセルの方程式は、<br>
T^* V^*=V^*<br>
と書き換えられる。<br>
ここで、0<\gamma<1 のとき、このベルマン最適作用素は、縮小写像となる。<br>
従って、この（最適状態価値関数に対する）ベルマンの方程式は、不動点方程式であり、不動点としての唯一の最適状態価値関数 V^*(s) の存在が保証される。<br>
※ このベルマン最適作用素は、max 演算により、非線形作用となっている。<br>

- （証明略）先の「状態価値関数」の項目を参照<br>


【定理】（状態価値関数に関しての）ベッセルの方程式<br>
マルコフ決定過程 M=(S,A,P) において、<br>
初期状態 s_0 と ”確率１の決定論的な” 定常方策 \pi \in \Pi_stat=1 から生じる確率過程 {s_t,a_t,r_t}_t=0^\infty に対して、状態価値関数 V^\pi(s) が定まるが、<br>
この行動価値関数 V^\pi(s) に対しては、以下のベルマンの方程式と呼ばれる関係が成り立つ。<br>
V^\pi(s) = R(s,\pi(s)) + \gamma * Σ_(s'\inS)( P(s,\pi(s),s')*V^\pi(s') ) (\forall s\in S)<br>
- a=\pi(s)=確率1<br>
ここで、このベルマンの方程式において状態価値関数を与える作用素を<br>
(T^\pi V) : s\in \doubleR^S → R(s,a) + \gamma * Σ_(s'\inS)( P(s,a,s')*V^\pi(s') )\in \doubleR^S<br>
を、ベルマン作用素という。このベルマン作用素を用いれば、ベッセルの方程式は、<br>
T^\pi V^\pi=V^\pi<br>
と書き換えられる。<br>
ここで、0<\gamma<1 のとき、このベルマン作用素は、縮小写像となる。<br>
従って、この（状態価値関数に対する）ベルマンの方程式は、不動点方程式であり、不動点としての唯一の状態価値関数 V^\pi(s) の存在が保証される。<br>
※ このベルマン作用素は、アフィン作用素であり、例えば行列で表現される線形作用素である。<br>

- （証明略）先の「状態価値関数」の項目を参照<br>

<br>

同様にして、最適行動価値関数 Q^*(s,a) に対してもベッセルの方程式が成り立ち、この不動点方程式の収束解（＝不動点）としての最適行動価値関数の値の存在性が保証される。<br>

【定理】（最適行動価値関数に関しての）ベッセルの方程式<br>
マルコフ決定過程 M=(S,A,P) において、<br>
初期状態 s_0 と初期行動 a_0、定常方策 \pi\in \Pi_stat から生じる確率過程 {s_t,a_t,r_t}_t=0^\infty に対して、状態行動関数 Q^\pi(s,a) が定まるが、<br>
この最適行動価値関数 Q^*(s,a) に対しては、以下のベルマンの方程式と呼ばれる関係が成り立つ。<br>
Q^*(s,a) = R(s,a) + \gamma * Σ_(s'\inS) ( P(s,a,s')* max_(a'\inA)Q^*(s,a') ) (\forall s\in S)<br>

ここで、このベルマンの方程式において行動価値関数を与える作用素を<br>
T^* Q^*:(s,a)\in \doubleR^(S\times A) → R(s,a) + \gamma *Σ_(s'\inS)( P(s,a,s')* max_(a'\inA)Q^*(s,a') ) \in \doubleR^S<br>
を、ベルマン最適作用素という。このベルマン最適作用素を用いれば、ベッセルの方程式は、<br>
T^* Q^*(s,a)=Q^*(s,a)<br>
と書き換えられる。<br>
ここで、0<\gamma<1 のとき、このベルマン最適作用素は、縮小写像となる。<br>
従って、この（最適行動価値関数に対する）ベルマンの方程式は、不動点方程式であり、不動点としての唯一の最適行動価値関数 Q^*(s,a) の存在が保証される。<br>
※ このベルマン最適作用素は、max 演算により、非線形作用となっている。<br>
