...
### ◎ ベルマン方程式
先に見たように、行動価値関数 Q^* に関してグリーディな方策（＝状態 s において Q を最大化する行動選択）は最適であるので、最適方策 π^* を求めるには、最適行動価値関数 Q^* の情報、或いは、最適状態価値関数 V^*・即時報酬 r・遷移確率 P の情報があればよいことになる。<br>

従って、次の問題は、これら最適価値関数 Q^*,V^* を如何にして求めるかということになる。<br>
この内、最適状態価値関数 V^* は、以下のベッセルの方程式と呼ばれる、V^* に関しての不動点方程式から求めることが原理的には可能となる。<br>
...
【定義】（最適状態価値関数に関しての）ベッセルの方程式<br>
マルコフ決定過程 M=(S,A,P) において、<br>
最適な行動価値関数 V^* に対して、以下のベルマンの方程式と呼ばれる不動点方程式が成り立つ。<br>
V^*(s) = max_(a\inA){ R(s,a) + γ*Σ( P(s,a,s')*V^*(y) ) }<br>
ここで、このベルマンの方程式において最適状態価値関数を与える作用素<br>
T^*:(T^*V)(s)\inR^S → max_(a\inA){ R(s,a) + γ*Σ( P(s,a,s')*V^*(y) ) }<br>
を、ベルマン最適作用素という。<br>
このベルマン最適作用素を用いれば、
※ このベルマンの方程式は、不動点方程式であり、不動点としての最適状態価値関数の存在が保証される。<br>
※ このベルマン最適作用素は、max 演算により、非線形作用となっている。<br>

<br>

【定義】（最適行動価値関数に関しての）ベッセルの方程式<br>
